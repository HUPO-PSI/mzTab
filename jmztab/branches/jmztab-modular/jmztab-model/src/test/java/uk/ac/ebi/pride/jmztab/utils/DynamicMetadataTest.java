...
public class DynamicMetadataTest {
    ...
    @Test
    public void testModifySoftwareId() throws Exception {
        DynamicMetadata mtd = new DynamicMetadata(getMetadata());
        mtd.modifySampleId(1, 4);

        SortedMap<Integer, Sample> sampleMap = mtd.getMetadata().getSampleMap();
        assertFalse(sampleMap.containsKey(1));
        assertTrue(sampleMap.containsKey(4));
        ...
        for (StudyVariable studyVariable : mtd.getMetadata().getStudyVariableMap().values()) {
            assertFalse(studyVariable.getSampleMap().containsKey(1));
        }
    }

    @Test
    public void testModifyMsRunId() throws Exception {
        DynamicMetadata mtd = new DynamicMetadata(getMetadata());

        mtd.modifyMsRunId(1, 4);

        SortedMap<Integer, MsRun> msRunMap = mtd.getMetadata().getMsRunMap();
        assertFalse(msRunMap.containsKey(1));
        assertTrue(msRunMap.containsKey(4));
