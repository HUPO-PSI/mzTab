...
/**
 *
 * MZTabFileParser provides reading functionality of the mzTab file. During the parsing process, minimal
 * integrity checks are preformed.
 *
 * @author qingwei
 * @since 21/02/13
 */
//TODO add integrityChecker
public class MZTabFileParser {
    ...
    public MZTabFileParser(File tabFile, OutputStream out) throws IOException {
        this(tabFile, out, LEVEL, MAX_ERROR_COUNT, true);
    }
     ...
     * @param out the output stream for parsing messages
     * @param level the minimum error level to report errors for
     * @param maxErrorCount the maximum number of errors to report in the {@link MZTabErrorList} return by
     * {@link MZTabFileParser#getErrorList()}
     * @throws java.io.IOException
     */
    public MZTabFileParser(File tabFile, OutputStream out, MZTabErrorType.Level level, int maxErrorCount, boolean integrityCheck) throws IOException {
        ...
        try {
            ...
            if(integrityCheck) {
                MZTabIntegrityChecker integrityChecker = new MZTabFileIntegrityChecker(mzTabFile, errorList);
        ...
        if (errorList.isEmpty()) {
            out.write(("MZTabFileParser: no errors in " + tabFile + " file!" + NEW_LINE).getBytes());
        }
    }
     ...
     */
    private void parse() throws IOException, MZTabException, MZTabErrorOverflowException {
